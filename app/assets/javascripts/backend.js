  ...
  $('.selectize-mieter').selectize({
      create: function(input,callback){
        ...
        $(".addMieter").on("click", function(e){
          ...
          $.ajax({
            ...
            url: "/mieters",
            dataType: 'json',
            data: {mieter: {vorname: vorname, name: name, email: email, telefon: telefon} },
            success: function(response) {
              callback({value: response.id, text: `ID: ${response.id} / ${response.vorname} ${response.name}`});
      ...
      render: {
        option_create: function(data, escape) {
          return '<div class="create">Neuen Mieter erstellen: <strong>' + escape(data.input) + '</strong>&hellip;</div>';
        }
      }
  });
